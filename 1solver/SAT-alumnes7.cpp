...
void OLSetup() {
  ...
  conflictes = vector<int>(numVars+1,0);
...
bool propagateGivesConflict ( ) {
  while ( indexOfNextLitToPropagate < modelStack.size() ) {
    ...
    for(int p = 0; p < OL[lit].size(); ++p) {
      ...
      int numUndefs = 0;
      int lastLitUndef = 0;
      vector<int> vals = vector<int>(clauses[i].size());
      for (uint k = 0; not someLitTrue and k < clauses[i].size(); ++k) {
        vals[k] = abs(clauses[i][k]);
      ...
      if (not someLitTrue and numUndefs == 0) {
        /*for(int k = 0; k < vals.size(); k++) {
          if(conflictes[vals[k]] < INT_MAX) conflictes[vals[k]]++;
        }*/
        conflictes[vals[0]]++;
        conflictes[vals[1]]++;
        conflictes[vals[2]]++;
        ...
        if(counter > 1000) {
          counter = 0;
          for(int i = 1; i < numVars+1; i++) conflictes[i] /= 2;
        }
...
int getNextDecisionLiteral(){
  ...
  for(int i = 1; i < numVars+1; i++) {
    if(currentValueInModel(i) == UNDEF) {
      if(conflictes[i] >= max) {max = conflictes[i]; maxVal = i; }
    }
  }
