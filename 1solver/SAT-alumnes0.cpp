...
uint numVars;
uint numClauses;
vector<vector<int> > clauses; //problema
vector<int> model; //solució en progress, cada literal té UNDEF, TRUE o FALSE
vector<int> modelStack; // ordre dels literals que s'han definit
uint indexOfNextLitToPropagate; // per quan s'han de propagar molts literals a la vegada
uint decisionLevel; // mida del modelStack
...
/*
 * Utilitzar les occurs lists per propagar de manera més eficient.
 */
...
void OLSetup() {
  vector< vector<int> > aux(numVars*2, vector<int>(numClauses));
}
