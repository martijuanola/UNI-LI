...
doubles:- 
    round(R1), R1 < 13,  R2 is R1+1,
    findall([-match(S,T1,R1),-match(S,T2,R2), double(S,R2)],
    (team(S), otherTeam(S,T1), otherTeam(S,T2))
    ,Lits),
    member(Lit, Lits), writeClause(Lit),
    fail.
doubles.
...
noDoubles:- 
    noDoubles(ND), findall(double(S,R), (team(S), member(R,ND)), Lits), exactly(0,Lits),
    fail.
noDoubles.

atmostOneTVMatchPerRound:- 
    round(R), findall(match(S,T,R), tvMatch(S-T), Lits), atMost(1, Lits),
    fail.
atmostOneTVMatchPerRound.

sixOrSevenHomes:-  
    team(T), findall(home(T,R), round(R), Lits), atLeast(6,Lits), atMost(7,Lits),
    fail.
sixOrSevenHomes.

atMostOneDouble:-  
    team(T), findall(double(T,R), round(R), Lits), atMost(1,Lits),
    fail.
atMostOneDouble.
