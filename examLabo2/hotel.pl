...
%% satVariable( gangsterAssigned(G) ):- gangster(G).  % means "gangster G has a room assigned".
...
writeClauses(NumUnAssigned):-
    ...
    atMostOneGangsterPerRoom,
    noEnemyNeighbour,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneRoomPerGangster(NumAssigned):- gangster(G), G =< NumAssigned, findall(gr(G,R) ,room(R) ,Lits), exactly(1,Lits),fail.
exactlyOneRoomPerGangster(_).

atMostOneGangsterPerRoom:- room(R), findall(gr(G,R) , gangster(G), Lits), atMost(1, Lits), fail.
atMostOneGangsterPerRoom.

noEnemyNeighbour:- enemies(G1,G2), neighborRooms(R1,R2), writeClause([-gr(G1,R1), -gr(G2,R2)]), fail.
noEnemyNeighbour.
...
costOfThisSolution(M,Cost):- findall(G, member(gr(G,_), M),Gangsters), length(Gangsters,NumG), numGangsters(N), Cost is N-NumG, !.
