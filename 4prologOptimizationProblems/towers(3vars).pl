...
satVariable( villageWithTower(V) ):- village(V).
satVariable( watched(V)  ):- village(V).  % means "position village V is watched"
...
writeClauses(MaxNumTowers):-
    villageWithTowerRule,
    watchedRule,
    watchedRule2,
    atMostMaxTowers(MaxNumTowers), %check
    atLeastOneTowerForSignificantVillage, %check
    atMostOneTowerForVillage, %check
    atLeastOneWatchedForVillage,
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


villageWithTowerRule:-
    village(V), posVillage(V,I,J),
    writeClause([-towerPos(I,J),villageWithTower(V)]), fail.
villageWithTowerRule.


watchedRule:-
    village(V), posVillage(V,I,J), posWatchesVillage(V2,I,J),
    V \= V2,
    writeClause([-towerPos(I,J), watched(V2)]), fail.
watchedRule:-
    village(V), posVillage(V,I,J),
    writeClause([-towerPos(I,J), watched(V)]), fail.
watchedRule.

watchedRule2:-
    village(V), 
    findall(towerPos(I,J), (posWatchesVillage(V,I,J), posVillage(V2,I,J), village(V2)), Lits),
    writeClause([-watched(V)|Lits]), fail.
watchedRule2.

atMostMaxTowers(MaxNumTowers):-
    findall(villageWithTower(V), village(V), Lits),
    atMost(MaxNumTowers, Lits), fail.
atMostMaxTowers(_).


atLeastOneTowerForSignificantVillage:-
    significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits),
    atLeast(1, Lits), fail.
atLeastOneTowerForSignificantVillage.

    
atMostOneTowerForVillage:-
    village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits),
    atMost(1, Lits), fail.
atMostOneTowerForVillage.


atLeastOneWatchedForVillage:- 
    findall(watched(V), village(V), Lits),
    member(Lit, Lits), writeClause([Lit]), fail.
atLeastOneWatchedForVillage.
...
costOfThisSolution(M,Cost):- findall((I,J), member(towerPos(I,J), M), Towers), length(Towers,Cost), !.
