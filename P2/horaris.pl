...
noOverlapCoursesSameYear:- courseYear(C1,Y), courseYear(C2,Y), C1 \= C2, day(D), hour(H), writeClause([-cdh(C1,D,H), -cdh(C2,D,H)]), fail.
noOverlapCoursesSameYear.

noOverlapProfs:- course(C1), course(C2), C1 \= C2, professor(P), day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.

noOverlapRooms:- course(C1), course(C2), C1 \= C2, room(R), day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ]), fail. 
noOverlapRooms.

relateVarsCPwithUsedProf:- professor(P), findall( cp(C,P), course(C), Lits ), expressOr(usedProf(P),Lits), fail.
relateVarsCPwithUsedProf.

maxProfs(MaxNumProf):- findall( usedProf(P), professor(P), Lits ), atMost(MaxNumProf,Lits), fail.
maxProfs(_).
