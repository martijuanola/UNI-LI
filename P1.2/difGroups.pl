...
satVars:- 
	student(S1), student(S2), team(T),
	findall([-st(S1,T), -st(S2,T), sst(S1,S2,T)], S1 \= S2, Lits),
	member(Lit, Lits), writeClause(Lit), fail.
satVars.

eachSexactly1T:- student(S), findall(st(S,T), team(T), Lits), exactly(1,Lits), fail.
eachSexactly1T.


eachTexactly5S:- teamSize(N), team(T), findall(st(S,T), student(S), Lits), exactly(N,Lits), fail.
eachTexactly5S.


atMost1Incompatible:- student(S1),
	findall(sst(S1,S2,T), (team(T), incompatible(S1,S2), S1 \= S2), Lits), atMost(1,Lits), fail.
atMost1Incompatible.
...
displaySol(_) :- nl, incompatible(S1,S2), write(S1), write(" "), write(S2), nl, fail.
