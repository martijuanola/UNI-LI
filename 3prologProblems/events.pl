...
writeClauses:-  
    possibleDaysAndEventsPerModerator,
    eachEexactlyOneD,
    eachEexactlyOneM,
    eachDatMostMaxE,
    eachMatMostMaxD,
...
%%em(E,M) and ed(E,D) -> md(M,D)
%%sese això, sense cap at least no es crea cap md(M,D) i 
%%eachMatMostMaxD no serverix de res després
possibleDaysAndEventsPerModerator:- 
    moderator(M), day(D), event(E), 
    writeClause([-em(E,M), -ed(E,D), md(M,D)]),
    fail.
possibleDaysAndEventsPerModerator.


eachEexactlyOneD:- event(E), findall( ed(E,D), (eventDays(E,DS), member(D,DS)), Lits), exactly(1,Lits), fail.
eachEexactlyOneD.


eachEexactlyOneM:- event(E), findall(em(E,M), (eventModerators(E,MS), member(M,MS)), Lits), exactly(1,Lits), fail.
eachEexactlyOneM.


eachDatMostMaxE:- maxEventsPerDay(MAX), day(D), findall(ed(E,D), (eventDays(E,DS), member(D,DS)), Lits), atMost(MAX, Lits), fail.
eachDatMostMaxE.


eachMatMostMaxD:- maxDaysPerModerator(MAX), moderator(M), 
    findall( md(M,D), 
        day(D),
    Lits), atMost(MAX, Lits), fail.
eachMatMostMaxD.
