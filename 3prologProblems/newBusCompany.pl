...
writeClauses:-  
    ...
    atMostOneTripPerDay,
    atLeastXTripsPerWeek,
    maxKmPerBus,
    nextCity,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneTripPerBusAndDay:- bus(B), day(D), findall(bdcc(B,D,C1,C2), trip(C1-C2), Lits), exactly(1,Lits), fail.
exactlyOneTripPerBusAndDay.

atMostOneTripPerDay:- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), Lits), atMost(1,Lits), fail.
atMostOneTripPerDay.
  
atLeastXTripsPerWeek:- frequency(C1-C2,F), findall(bdcc(B,D,C1,C2), (bus(B), day(D)), Lits), atLeast(F,Lits), fail.
atLeastXTripsPerWeek.

maxKmPerBus:- tooLongDist(C1-C2-C3),
    findall([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C2,C3)], (bus(B), consecutiveDays(D1,D2)), Lits),
    member(Lit,Lits), writeClause(Lit), fail.
maxKmPerBus.

nextCity:- bus(B), consecutiveDays(D1,D2), 
    findall([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C3,C4)], (trip(C1-C2),trip(C3-C4),C2 \= C3), Lits),
    member(Lit,Lits), writeClause(Lit), fail.
nextCity.
