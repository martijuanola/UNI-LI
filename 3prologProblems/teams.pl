...
eachWexactlyOneT:- worker(W), findall( wt(W,T), team(T), Lits), exactly(1,Lits), fail.
eachWexactlyOneT.


eachTatLeastMinW:- minSize(MIN), team(T), findall( wt(W,T), worker(W), Lits), atLeast(MIN,Lits), fail.
eachTatLeastMinW.


eachTatMostMaxW:- maxSize(MAX), team(T), findall( wt(W,T), worker(W), Lits), atMost(MAX,Lits), fail.
eachTatMostMaxW.


eachWWinTisCompatible:- team(T),
    findall(
        [-wt(W1,T),-wt(W2,T)],
        (worker(W1) ,worker(W2), W1 > W2, incompatibleWorkers(W1,W2)),
        LitList),
    member(Lit,LitList), atLeast(1,Lit), fail.
eachWWinTisCompatible.
