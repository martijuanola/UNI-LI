...
exactly1PosPerCar:- car(C), findall(carPos(C,P), position(P), Lits), exactly(1,Lits), fail.
exactly1PosPerCar.


exactly1CarPerPos:- position(P), findall(carPos(C,P), car(C), Lits), exactly(1,Lits), fail.
exactly1CarPerPos.


relateCarPosAndPosOpt:- car(C), position(P),
    findall([-carPos(C,P), posOpt(P,Opt)], carOption(C,Opt) , Lits),
...
optionConstraints:-
    ...
    windowOfSize(WindowSize,I,F),
    findall(posOpt(P,Opt), (position(P), between(I,F,P)), Lits),
